...
## Create the StoreDatabase and CartContainer

Before running the data generator, you need to create the database and container that will store the shopping cart actions.
...
3. At the top of the **Data Explorer** section, select **New Container**.

4. In the **New Container** pane on the right, enter the following values:

   - **Database id**: Select **Create new** and enter `StoreDatabase`
   - **Container id**: Enter `CartContainer`
   - **Partition key**: Enter `/Item` (note the capital "I")
   - **Throughput**: Select **Manual** and enter `400` RU/s

5. Select **OK** to create the database and container.

   > **Important**: The partition key must be `/Item` (with a capital "I") because the CartAction objects generated by the data generator use the `Item` property (e.g., "Women's Red Dress") as the partition key value. Using a different partition key will result in errors when inserting documents.
...
### Create the Destination Container for Live Migration

Before setting up the change feed processor, you need to create the destination container that will receive the migrated data with a different partition key.

1. In the Azure Portal, navigate to your Azure Cosmos DB account.

2. From within the **Azure Cosmos DB** blade, select the **Data Explorer** tab on the left.

3. Expand the **StoreDatabase** database, then select the **...** (more options) button next to it.

4. Select **New Container**.

5. In the **New Container** pane on the right, enter the following values:

   - **Database id**: Select **Use existing** and choose `StoreDatabase`
   - **Container id**: Enter `CartContainerByState`
   - **Partition key**: Enter `/BuyerState` (note the capital "B" and "S")
   - **Throughput**: Select **Manual** and enter `400` RU/s

6. Select **OK** to create the container.

   > **Important**: This container uses `/BuyerState` as the partition key, which is different from the source `CartContainer` that uses `/Item`. The change feed processor will migrate data from one partition key scheme to another in real-time.
...
3. For the `_endpointUri` variable, replace the placeholder value with the **URI** value from your Azure Cosmos DB account. This lab uses Azure AD authentication via `DefaultAzureCredential`, so you'll need to ensure you're logged in with `az login --tenant <your-tenant-id>`.

   > **Note**: Make sure to add `using Azure.Identity;` at the top of the file and ensure the Azure.Identity NuGet package is installed in the project.
...
6. Now, add the following lines of code directly after the **leaseContainer** definition in order to get an instance of the change processor:
   ...
   > Each time a set of changes is received, the lambda function defined in `GetChangeFeedProcessorBuilder` will be called. The code loops through each `CartAction` document and writes it to the destination container using `BuyerState` as the partition key.
...
> **Note**: The change feed processor code has already been completed in the previous steps. The migration logic is now part of the lambda function that processes changes. This section documents what was implemented.

1. The **input** parameter in the `GetChangeFeedProcessorBuilder` lambda is a collection of **CartAction** documents that have changed. The code loops through each document and writes it to the destination container using `BuyerState` as the partition key:
   ...
   > By using `Task.WhenAll(tasks)`, all write operations are performed in parallel for better performance.
...
1. **Note**: If the `ChangeFeedFunctions` folder already exists in your Lab08 directory, **skip** the `func init` and `func new` commands below and proceed directly to updating the project file.
...
1. When prompted, choose the **dotnet (in-process model)** worker runtime. Use the arrow keys to scroll up and down.

   > **Important**: Choose the in-process model, not the isolated worker model.
...
1. Open the **ChangeFeedFunctions.csproj** file and update it to use .NET 8 and Azure Functions v4:
   ...
   <Project Sdk="Microsoft.NET.Sdk">
     ...
     <ItemGroup>
       ...
       <PackageReference Include="Microsoft.Azure.WebJobs.Extensions.CosmosDB" Version="4.11.0" />
   ...
   > **Note**: The newer versions of packages are required for .NET 8 compatibility. If you see build errors about package downgrades, ensure you're using the exact versions shown above.
...
1. In your terminal pane, build the project:
   ...
   > **Troubleshooting**: If you see errors about missing `Microsoft.Azure.Functions.Worker.Builder`, delete the `ChangeFeedFunctions/ChangeFeedFunctions/` nested folder if it exists (this is created by the isolated worker template but not needed for in-process functions).
...
1. Select the new `MaterializedViewFunction.cs` file to open it in the editor.
   ...
   > The **databaseName**, **containerName** and **Connection** refer to the source Cosmos DB account that the function is listening for changes on.

1. Change the **databaseName** value to `StoreDatabase`

1. Change the **collectionName** parameter to **containerName** and set its value to `CartContainerByState`

   > **Note**: In Azure Functions v4, the parameter name changed from `collectionName` to `containerName`.
...
1. Between **Connection** and **LeaseContainerName** add the following line:
   ...
   > **Note**: In Azure Functions v4, `CreateLeaseCollectionIfNotExists` is now `CreateLeaseContainerIfNotExists`.

1. Change the **LeaseCollectionName** parameter to **LeaseContainerName** and set the value to `materializedViewLeases`

   > Lease containers are a critical part of the Cosmos DB Change Feed. They allow multiple instances of a function to operate over a container and serve as a virtual _bookmark_ for where the function last left off.

1. Change the input parameter type from `IReadOnlyList<Document>` to `IReadOnlyList<dynamic>`

   > **Note**: In newer versions, `Document` is internal. Use `dynamic` or strongly-typed models like `CartAction`.
...
1. Following the definition of the **CartAction** class, add a new class as follows:
   ...
   > This class will be used to store aggregated sales data by state in the StateSales container.
   public class StateCount
   {
      [JsonProperty("id")]
      public string Id { get; set; }
